...
- (void)testIfNumberOfEventsIsExactlyFifteen {
    XCTestExpectation *expectation = [self expectationWithDescription:@"Waiting for comments to return"];

    [Event performSearchWithKeyword:@"mobile" andComplete:^(NSArray *events) {
        XCTAssertTrue(events.count == 15);
        [expectation fulfill];
    }];
...
- (void)testIfSecondEventHasOnlyOneCommentAndCommentWasCreatedBySpecificUser {
    ...
    [Event performSearchWithKeyword:@"mobile" andComplete:^(NSArray *events) {
        ...
        [secondEvent getCommentsWithBlock:^(NSArray *comments) {
            ...
            int memberIdInt = [comment.memberID intValue];
            XCTAssertTrue(memberIdInt == 99045732);
        }];
    }];
}
